export function cardinalityFromMinMax(min, max) {
    const lo = min ?? -1;
    const hi = max ?? -1;
    if (lo === -1 && hi === -1)
        return undefined;
...
export function formatMemberCardinality(card) {
    if (!card || card === "*")
        return "*";
    if (card === "+")
        return "+";
...
export function formatGlobalCardinality(card) {
    if (card === "*")
        return "0..*";
    if (card === "+")
        return "1..*";
...
export function parseCardinalityToken(raw) {
...
export function cardinalityToMinMax(card) {
    if (!card || card === "*")
        return { min: 0, max: -1 };
    if (card === "+")
        return { min: 1, max: -1 };
